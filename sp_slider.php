...
<body>
	...
	//getting username of renter
	$sql="SELECT renteduser FROM vehicleinfo where vehicleid=$id";
	$query=mysqli_query($connect,$sql);
	$data=array();
	while ($row=mysqli_fetch_assoc($query))
	{
	...
	foreach ($data as $key => $value)
	{
		$rentedusername=$value['renteduser'];	
	}	


	//getting availability of vehicle
	$sql="SELECT availability FROM vehicleinfo where vehicleid=$id";
	$query=mysqli_query($connect,$sql);
	$data=array();
	while ($row=mysqli_fetch_assoc($query))
	{
					...
					<div class="ad">
						...
						<div class="info">
							<table>
								...
								<?php
								if($availability=='Booked'|| $useremail==$value['email'])
								{
								 ?>
								 <tr><td>Rented by</td><td>:</td><td><?php echo $rentedusername;?></td></tr>
								<?php
							}
							else{
								?>
								<tr><td colspan="3" style="text-align: center;"> <a href="sp_book.php?vehicleid=<?php echo $value['vehicleid']?>&vehicleprice=<?php echo $vehicleprice?>">Book</a></td></tr>
								<?php
							}
							?>
							</table>
